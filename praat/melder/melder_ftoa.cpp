...
const char * Melder8_single (double value) {
	...
	if (isundef (value))
		return "--undefined--";
	fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.9g", value);
	return buffers8 [ibuffer];
}
...
const char * Melder8_half (double value) {
	...
	if (isundef (value))
		return "--undefined--";
	fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.4g", value);
	return buffers8 [ibuffer];
}
...
const char * Melder8_fixed (double value, integer precision) {
	...
	const int n = fmt_snprintf (buffers8 [ibuffer], MAXIMUM_NUMERIC_STRING_LENGTH + 1, "%.*f",
...
const char * Melder8_fixedExponent (double value, integer exponent, integer precision) {
	...
	value /= factor;
	const int minimumPrecision = - (int) floor (log10 (fabs (value)));
	const int n = fmt_snprintf (buffers8 [ibuffer], MAXIMUM_NUMERIC_STRING_LENGTH + 1, "%.*fE%d",
...
const char * Melder8_hexadecimal (integer value, integer precision) {
	...
	const int n = fmt_snprintf (buffers8 [ibuffer], MAXIMUM_NUMERIC_STRING_LENGTH + 1, "%.*llX",
...
const char * Melder8_dcomplex (dcomplex value) {
	...
	if (isundef (value.real()) || isundef (value.imag()))
		return "--undefined--";
	fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.15g", value.real());
	if (Melder8_strtod (buffers8 [ibuffer], nullptr) != value.real()) {
		fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.16g", value.real());
		if (Melder8_strtod (buffers8 [ibuffer], nullptr) != value.real())
			fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.17g", value.real());
	}
...
const char * Melder8_scomplex (dcomplex value) {
	...
	if (isundef (value.real()) || isundef (value.imag()))
		return "--undefined--";
	fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.9g", value.real());
	char *p = buffers8 [ibuffer] + strlen (buffers8 [ibuffer]);
	*p = ( value.imag() < 0.0 ? '-' : '+' );
...
const char * Melder8_naturalLogarithm (double lnNumber) {
	...
	if (log10Number < -41.0) {
		...
		fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.15g", remainder10);
		if (Melder8_strtod (buffers8 [ibuffer], nullptr) != remainder10) {
			fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.16g", remainder10);
			if (Melder8_strtod (buffers8 [ibuffer], nullptr) != remainder10)
				fmt_snprintf (buffers8 [ibuffer],MAXIMUM_NUMERIC_STRING_LENGTH+1, "%.17g", remainder10);
		}
