...
autoTextTier TextTier_readFromXwaves (MelderFile file) {
	try {
		...
		for (;;) {
			...
			if (p == line || sscanf (p, "%td%299s", & colour, mark) < 2) // if (sscanf (line, "%lf%td%299s", & time, & colour, mark) < 3)   // BUG: buffer overflow
		...
		*/
		for (;;) {
			...
			if (p == line)
				...
				numberOfElements = sscanf (p, "%td%199s", & colour, mark) + 1;
