...
def test_harmonicity(resources):
    ...
    assert harmonicity[-1] == harmonicity[len(harmonicity) - 1]
    ...
    print(
        f"Minimum={harmonicity.get_minimum()} dB @ {harmonicity.get_time_of_minimum()}"
    )
    print(
        f"Maximum={harmonicity.get_maximum()} dB @ {harmonicity.get_time_of_maximum()}"
    )
